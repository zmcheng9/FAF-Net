...
from models.fewshot_fafnet import FewShotSeg
...
def main(_run, _config, _log):
    ...
    criterion_focal = FocalLoss(gamma=0.0, weights=my_weight)
    ...
    log_loss = {'total_loss': 0, 'query_loss': 0, 'align_loss': 0, 'proto_loss': 0}
    ...
    for sub_epoch in range(n_sub_epochs):
        ...
        for _, sample in enumerate(train_loader):
            ...
            query_pred, align_loss, proto_loss = model(support_images, support_fg_mask, query_images, train=True)
            query_pred = torch.clamp(query_pred, torch.finfo(torch.float32).eps, 1 - torch.finfo(torch.float32).eps)
            query_loss = criterion_focal(query_pred.permute(0, 2, 3, 1), query_labels)

            loss = query_loss + align_loss + proto_loss
            ...
            if (i_iter + 1) % _config['print_interval'] == 0:
                ...
                _log.info(f'step {i_iter + 1}: total_loss: {total_loss}, query_loss: {query_loss}, proto_loss: {proto_loss}'
